...
function getEvents(hoursToCheckFor) {
    let calendars = [];
    let todayEvents = [];
    // ***** GOOGLE CALENDAR API *****
  ...
  fs.readFile('./bin/credentials.json', (err, content) => {
    ...
    authorize(JSON.parse(content), getAllCals);
  });
  ...
  function getAccessToken(oAuth2Client, callback) {
     ...
     */
     function getAllCals(auth) {
        const calendar = google.calendar({version: 'v3', auth});
        calendar.calendarList.list({ auth }, function(err, resp) {
            ...
            calendars.forEach(function(cal) {
                let date = new Date();
                date.setTime(date.getTime() + (hoursToCheckFor*60*60*1000));
                date.toISOString();
